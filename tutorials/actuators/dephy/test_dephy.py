...
def main():
    ...
    if control_type in control_types:
        ...
        print("Please enter a valid control type. Valid control types are 'Current', 'Position', 'Voltage', 'Torque', 'Impedance'")
...
def test_impedance_control(actpack: Dephy.DephyActuator):
    with actpack:
        try:
            
            actpack.update()
            _prompt_to_continue(actpack, 'impedance')
            ...
            k = [0, 0, 0, 0]
            b = [0, 0, 0, 0]
            angles = np.array([1/8, 1/4, 1/2, 3/4, 1]) * 3.14
            ...
            for i in range(len(angles)):
                ...
                while (abs(actpack.output_velocity) >= 0.01):
                    actpack.set_output_position(value=current_position + angles[i])

                _log_current_state(actpack, 'Impedance', k[i])
                time.sleep(1)

        except KeyboardInterrupt:
            LOGGER.exception("KeyboardInterrupt while setting motor impedance")  
            exit()
